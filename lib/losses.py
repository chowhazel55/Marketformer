...
def masked_mse_loss_nasdaq(pred, label, null_val):
    ...
    base = label[:, 1]
    ground_truth = label[:, 0]
    return_ratio = (pred - base) / (base + 1e-12)
    diff = (return_ratio - ground_truth) ** 2
    diff = diff * mask  # 只保留 mask=1 的
    loss = diff.sum() / (mask.sum() + 1e-12) # 除以有效样本数
    return loss
